...
public class MessagesActivity extends Activity
{
	...
	private void showMessages()
	{
		...
		if(connectionDetector.isConnectedToInternet())
			new ODeskgetMessagesTask().execute();
		else
	...
	class ODeskgetMessagesTask extends AsyncTask<Void, Void, String> 
	{
		...
		protected void onPreExecute() 
		{
			...
			_dialog.setMessage("Getting Messages From Odesk");
		...
		protected void onPostExecute(String result) 
		{
			...
			if (_dialog.isShowing()) 
			{
				...
				if(list.size()>0)
				{
					...
					{
						LinearLayout linearLayout = (LinearLayout)getLayoutInflater().inflate(R.layout.message_item, null);


						LinearLayout msgitem = (LinearLayout)linearLayout.findViewById(R.id.msgitem);
						TextView name = (TextView)linearLayout.findViewById(R.id.name);
						TextView subject = (TextView)linearLayout.findViewById(R.id.subject);
						TextView message = (TextView)linearLayout.findViewById(R.id.message);

						if(info.getUnread()==Config.KEY_1)
						{
							linearLayout.setBackgroundColor(Color.parseColor("#DEDCDC"));
						}
						...
						messages_linearlayout.addView(linearLayout);
					}
				}
				else
				{
					...
					tx_noitems.setText(Config.KEY_MESSAGES_EMPTY);
